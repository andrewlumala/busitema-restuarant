...
| `/api/wallet/topup-direct` | POST | Student | Start a direct-to-merchant-code top-up (USSD, no API) |
| `/api/wallet/pending-topups` | GET | Admin/Cashier | Mobile money top-ups awaiting confirmation |
| `/api/wallet/confirm-topup/:id` | POST | Admin/Cashier | Confirm a direct top-up actually arrived |
| `/api/wallet/admin-topup` | POST | Admin/Cashier | Credit a student's wallet for cash handed over in person |
...
## Direct-to-merchant-code payment (no API needed)

Since you don't have MTN/Airtel API credentials yet, wallet top-ups can happen two ways that
don't need any provider integration at all:

1. **Direct USSD** (`/topup-direct`) — the student's own phone dials the real merchant code
   directly (MTN `*165*3#` / Airtel `*185*9#`), confirmed with their own PIN. Our backend never
   touches this transaction — it just shows the merchant code, the amount, and a button that
   opens the phone dialer pre-loaded with the right USSD code. **Important limitation:** USSD
   menus are interactive (dial → enter code → enter amount → enter PIN), and there's no
   reliable way to pre-fill the merchant code and amount into one link across every phone and
   carrier — so the student still types those two values in manually at the prompts. The link
   only saves them looking up and dialing the code themselves.

2. **Cash to admin** (`/admin-topup`) — student hands over physical cash, staff credits the
   wallet immediately (no confirmation step needed, since receiving the cash *is* the
   confirmation).

Because #1 happens entirely outside your system, **there's no way to know it succeeded without
